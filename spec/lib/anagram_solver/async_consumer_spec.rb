...
describe AnagramSolver::AsyncConsumner do
  let(:alien) { AnagramSolver::AsyncConsumner.new }

  context "Initialization" do

    context "Block" do
      let(:bob) do
        AnagramSolver::AsyncConsumner.
          new { |hiya| hiya }
      end
    ...
    context "Consumering" do
      ...
      it "must consume data in queue" do
        me = ->(*args) { }
        pet = AnagramSolver::AsyncConsumner.new(&me)
        pet.push [ :test ]
        pet.consume
      ...
      it "must call the block passed" do
        ...
        frankie = AnagramSolver::AsyncConsumner.new
    ...
    context "AsyncConsumner" do
      ...
      it "must be working" do
        ...
        sun = AnagramSolver::AsyncConsumner.
    ...
    describe "Avoiding Race condition" do
      ...
      context "Lock","with Mutex" do
        ...
        let(:lion) { AnagramSolver::AsyncConsumner.new }
        ...
        describe "while consuming" do
          ...
          it "must be set automaticaly based on queue's size" do
            kcolb = ->(args) { args }
            cat = AnagramSolver::AsyncConsumner.new(&kcolb)
            cat.push(:me_too)
